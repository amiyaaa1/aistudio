...
- 后端会根据密钥反查邮箱并路由到同邮箱的浏览器连接；若该账号未连接会返回 503。若只有一个浏览器在线但账号标签没配对，后端会自动把该连接重命名为请求的账号，避免误判未连接。
- 支持 OpenAI 兼容接口：
  - 列表模型：`GET /v1/models`。
...
- 如果对应账号的浏览器连接不存在或掉线，将返回 503，不会落到其他账号的浏览器上；仅在“只有一个浏览器在线但标签未对上”时，后端会自动把该连接重命名为请求账号，以避免误判。
